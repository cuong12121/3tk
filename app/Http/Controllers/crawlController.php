...
http://dienmaynguoiviet.com/dieu-hoa-Funiki-SH09MMC-2-chieu-9000BTU/";
    ...
    public function getLink()
    {
        $link = 'https://3tkshop.vn/giay-bao-ho-lao-dong-421.html';

        $dom = file_get_html($link);

        // $dom = $this->getDom('https://3tkshop.vn/bao-ve-dau-11.html');

        foreach ($dom->find('#getproducts .card .product-item') as $links) {


            DB::table('crawl_link')->insert(['link'=>$links->href, 'group_id'=>421, 'group_link'=>$link]);
        }
    ...
    public function getProductDetails()
    {
        ...
        foreach ($links_crawl as $key => $value) {
            ...
            $link = 'https://3tkshop.vn'.$value->link;

            $dom = file_get_html($link);

            $name = trim(strip_tags($dom->find('.pdetail-name h1',0)));

            $price = trim(strip_tags(str_replace('.', '', $dom->find('.pdetail-price-box h3 span',0))));

            $image =   str_replace('https://3tkshop.vn/', '', strip_tags($dom->find('#mygallery1 img',0)->src));

            $specifications = $dom->find('.pdetail-attrfeatured table',0);

            $details = str_replace($dom->find('.pdetail-attrfeatured table',0),'', $dom->find('.pdetail-attrfeatured',0));
